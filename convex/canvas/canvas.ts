...
// PROMOTE SEED TO CANVAS
export const promoteSeedToCanvas = mutation({
  args: {
    canvasId: v.id("canvas"),
    seedId: v.id("seeds"),
  ...
  handler: async (ctx, args) => {
    ...
    const canvas = await ctx.db.get(args.canvasId);
    if (!canvas) throw new Error("Canvas not found");
    if (canvas.userId !== user._id) throw new Error("Unauthorized");
    ...
    // Create a display node with the seed content
    const nodeId = `seed-${args.seedId}-${Date.now()}`;
    const newNode = {
      id: nodeId,
      type: "display",
      ...
      data: {
        id: nodeId,
        type: "text",
        content: seed.fullText || seed.summary || seed.title,
        createdAt: Date.now(),
        status: "complete",
        color: "#10b981", // Green color for KB seeds
        seedId: args.seedId,
        kbId: seed.kbId,
        metadata: {
          source: "knowledge_garden",
    ...
    // Add node to canvas
    const newNodes = [...canvas.nodes, newNode];
    ...
    // Create seed link to track promotion
    await ctx.runMutation(api.knowledge_garden.seedLinks.create, {
      seedA: args.seedId,
      seedB: args.seedId, // Self-reference for tracking (will be prevented by validation)
      relation: "USED_IN_FLOW",
      score: 1.0,
    });

    // Update seed to track canvas promotion
    await ctx.db.patch(args.seedId, {
      sourceFlowId: args.canvasId,
      updatedAt: Date.now(),
    });

    console.log(`[Canvas] Promoted seed ${args.seedId} to canvas ${args.canvasId} as node ${nodeId}`);
